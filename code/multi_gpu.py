from __future__ import print_function
import keras
from keras.models import*
from keras.layers import Input, merge, Lambda
from keras.layers.merge import Concatenate
from keras import backend as K

import tensorflow as tf
session_config = tf.ConfigProto()
session_config.gpu_options.allow_growth = True
session = tf.Session(config=session_config)

def slice_batch(x, gpus, part):
    sh = K.shape(x)
    L =  sh[0] // gpus
    if part == gpus - 1:
        return x[part*L:]
    return x[part*L:(part+1)*L]


def multi_gpu_model(model, gpus=1):
    if gpus ==1:
	   return model
    
    with tf.device('/cpu:0'):
        input_shape = tuple(model.input.get_shape().as_list())[1:]
        x = Input(input_shape)
    towers = []
    for g in range(gpus):
        with tf.device('/gpu:' + str(g)):
            slice_g = Lambda(slice_batch, lambda shape: shape, arguments={'gpus':gpus, 'part':g})(x)
            towers.append(model(slice_g))

    with tf.device('/cpu:0'):
        # Deprecated
 	#merged = merge(towers, mode='concat', concat_axis=0)
	merged = Concatenate(axis=0)(towers)

    new_model = Model(inputs=[x], outputs=merged)
    funcType = type(model.save)

    # monkeypatch the save to save just the underlying model
    def new_save(self_,filepath, overwrite=True):
            model.save(filepath, overwrite)
    new_model.save=funcType(new_save, new_model)
    return new_model